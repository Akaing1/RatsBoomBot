...
def create_broadcaster_subscriptions(broadcaster_user_id: str) -> list[Any]:
    ...
    channel_point_subscription = (
        create_channel_point_redemption_subscription(
            broadcaster_user_id
        )
    )
    ...
    raid_subscription = create_raid_subscription(
        broadcaster_user_id
    )
...
async def setup_database(db: asqlite.Pool) -> tuple[list[tuple[Any, Any]], list[Any], list[str]]:
    ...
    for row in rows:
        ...
        broadcaster_subscriptions = (
            create_broadcaster_subscriptions(user_id)
        )

        subscriptions.extend(broadcaster_subscriptions)
    ...
    LOGGER.info(
        "[Database] Loaded %d OAuth tokens.",
        len(tokens)
    )
    LOGGER.info(
        "[Database] Loaded %d broadcaster tokens.",
        len(broadcasters)
    )
    LOGGER.info(
        "[EventSub] Prepared %d total subscriptions.",
        len(subscriptions)
    )

    if not tokens:
        LOGGER.warning(
            "[OAuth] No OAuth tokens were found in the database."
        )

    if not broadcasters:
        LOGGER.warning(
            "[OAuth] No broadcaster tokens were found in the database."
        )
...
async def delete_token(db: asqlite.Pool, user_id: str) -> None:
    ...
    LOGGER.debug(
        "[OAuth] Deleting token for user %s.",
        user_id
    )
    ...
    LOGGER.info(
        "[OAuth] Deleted token for user %s.",
        user_id
    )

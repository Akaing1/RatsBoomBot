...
class MilkyGalaxyPointsCommands(ChannelComponent):
    ...
    @commands.group(name="dust", invoke_fallback=True, case_insensitive=True)
    async def dust(self, ctx: commands.Context, target: LocalizedUser = None) -> None:
    ...
    async def dust_add(self, ctx: commands.Context, target: LocalizedUser, amount: int) -> None:
        if not await self.require_feature(ctx, FeatureName.POINTS):
            return

        await self.handler.add_points(ctx, target, amount, "dust")

    @dust.command(name="give")
    async def dust_give(self, ctx: commands.Context, target: LocalizedUser, amount: int) -> None:
        if not await self.require_feature(ctx, FeatureName.POINTS):
            return

        await self.handler.give_points(ctx, target, amount, "dust")
    ...
    @dust.command(name="roulette", aliases=("spin",))
    async def dust_roulette(self, ctx: commands.Context, color: str = None, amount: str = None) -> None:
        if not await self.require_feature(ctx, FeatureName.POINTS):
            return

        await self.handler.roulette(ctx, color, amount, "dust")

    @dust.group(name="duel", invoke_fallback=True, case_insensitive=True)
    async def dust_duel(self, ctx: commands.Context, opponent: LocalizedUser = None, amount: str = None) -> None:
        if not await self.require_feature(ctx, FeatureName.POINTS):
            return

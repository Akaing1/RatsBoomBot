...
class SteohanyyPointsCommands(ChannelComponent):
    ...
    @commands.group(name="drinks", invoke_fallback=True, case_insensitive=True)
    async def drinks(self, ctx: commands.Context, target: LocalizedUser = None) -> None:
    ...
    async def drinks_add(self, ctx: commands.Context, target: LocalizedUser, amount: int) -> None:
        if not await self.require_feature(ctx, FeatureName.POINTS):
            return

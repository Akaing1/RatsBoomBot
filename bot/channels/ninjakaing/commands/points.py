...
class NinjakaingPointsCommands(ChannelComponent):
    ...
    @commands.group(name="bread", invoke_fallback=True, case_insensitive=True)
    ...
    @bread.command(name="give")
    async def bread_give(self, ctx: commands.Context, target: LocalizedUser, amount: int) -> None:
        if not await self.require_feature(ctx, FeatureName.POINTS):
            return

        await self.handler.give_points(ctx, target, amount, "bread")
    ...
    @bread.command(name="roulette", aliases=("spin",))
    async def bread_roulette(self, ctx: commands.Context, color: str = None, amount: str = None) -> None:
        if not await self.require_feature(ctx, FeatureName.POINTS):
            return

        await self.handler.roulette(ctx, color, amount, "bread")

    @bread.group(name="duel", invoke_fallback=True, case_insensitive=True)

...
class NinjakaingPointsCommands(ChannelComponent):
    ...
    async def bread(self, ctx: commands.Context, target: User = None) -> None:
    ...
    async def bread_add(self, ctx: commands.Context, target: User, amount: int) -> None:
        if not await self.require_feature(ctx, FeatureName.POINTS):
            return
    ...
    async def bread_duel(self, ctx: commands.Context, opponent: User = None, amount: str = None) -> None:
        if not await self.require_feature(ctx, FeatureName.POINTS):
            return

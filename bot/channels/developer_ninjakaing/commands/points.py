...
class DeveloperPointsCommands(ChannelComponent):
    ...
    @commands.group(name="ores", invoke_fallback=True, case_insensitive=True)
    ...
    @ores.command(name="roulette", aliases=("spin",))
    async def ores_roulette(self, ctx: commands.Context, color: str = None, amount: str = None) -> None:
        if not await self.require_feature(ctx, FeatureName.POINTS):
            return

        await self.handler.roulette(ctx, color, amount, "ores")

    @ores.group(name="duel", invoke_fallback=True, case_insensitive=True)
